...
function sumarTotalNeto() {
    ...
    if (tipoComprobante == "FACTURA") {
        $('#lblImpuesto').html("<b>ISV " + (impuesto * 100) + "% : </b>Q. " + impuestoTotal);
        $('#lblDescuento').html("<b>Descuento: </b>Q. " + parseFloat(descuentoTotal).toFixed(2));
        $('#lblSubtotal').html("<b>Sub Total: </b> Q. " + subTotal);
        $('#lblTotalNeto').html("<b>Total Neto: </b> Q. " + total.toFixed(2));
    } else {
        $('#lblTotalNeto').html("<b>Total Neto: </b> Q. " + total.toFixed(2));
    }
}
    ...
    $.ajax({
        ...
        if (resp > 0) {
            Swal.fire({
                ...
                text: "Datos guardados correctamente, nueva venta registrada, el cambio del cliente es: Q." + cambio,
                icon: 'success',
                showCancelButton: true,
                confirmButtonColor: '#3085d6',
                cancelButtonColor: '#d33',
