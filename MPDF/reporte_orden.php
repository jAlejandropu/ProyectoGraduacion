...
$query = " SELECT t.transaccion_fecha, 
    ...
    INNER JOIN persona p ON p.persona_id = t.persona_id
    ...
    <body>
      <header class="clearfix">
          ...
          <thead>
              <tr>
                  ...
                  <th width="25%" style="text-align: center;">
                    <h1 style="color: black;">REPORTE ORDEN </h1>
        ...
        <div id="project">
          ...
          <div><span style="color: black; font-size: 14px;"><b>Cliente: </b>'.$row['NOMBRE_USUARIO'].'</span></div>
      ...
      <main>
        <table border = "0">
          ...
          $contador = 0;  
          $sql = "call SP_LISTAR_DETALLE_ORDENR('".$row['transaccion_id']."')";
          $resultado2 = $mysqli->query($sql);
          while ($row2 = $resultado2->fetch_assoc()) {   
            $contador++;
            $estado = '';

            switch ($row2['prod_serv_tipo']) {
                case 'PRODUCTO':
                    $estado = '--';
                    break;
          ...
          $html.='
              ...
              <tr>
                <td colspan="6" style="background: #ffffff"><b>SUBTOTAL</b></td>
                <td colspan="2" class="total" style="background: #ffffff">Q. '.($row['transaccion_total'] - $row['transaccion_impuesto'] + $row['transaccion_descuento']) .'</td>
              </tr>
              <tr>
              
              <tr>
                <td colspan="6" style="background: #ffffff"><b>DESCUENTO</b></td>
                <td colspan="2" class="total" style="background: #ffffff">Q. '.$row['transaccion_descuento'] .'</td>
              </tr>
              <tr>
                <td colspan="6" style="background: #ffffff"><b>TOTAL NETO</b></td>
                <td colspan="2" class="grand total" style="background: #ffffff">Q. '.$row['transaccion_total'].'</td>
              </tr>';
